...
export const FINDINGS: Finding[] = [
  ...
  { n: 19, theme: "Execution", title: "39% win rate, still +24% OOS — payoff over hit-rate", body: "Funded $1,000, trading the validated edge out-of-sample with live AGGRESSIVE sizing: won only 13 of 33 (39.4%) yet finished $1,243.81 (+24.4%, −11.5% max DD), net of fees on real exchange data. Profit from payoff asymmetry, not hit-rate. An honest edge needs positive expectancy, not frequent wins.", isNew: true, tone: "honest" },
  ...
  { n: 21, theme: "Method", title: "Validation anchored on Mantle — auditable, not asserted", body: "When an edge clears the gate, its validation record (asset, edge, p_win, payoff, sample, OOS-ROI, firm ERC-8004 tokenId) is hashed and anchored in a Mantle tx. MNT OI-contrarian anchored live: tx 0x5ae63fd1…, block 39,509,049, SUCCESS — the calldata IS the record hash, recomputable by anyone. Earn → graduate → record on-chain. A tamper-proof proof-of-existence, not a trade.", isNew: true, tone: "neutral" },
  { n: 22, theme: "Alpha", title: "We tested ML too — no free pass", body: "Honoring 'an ML model per asset', we trained RandomForest + XGBoost on MNT features to predict 24h direction and traded their high-confidence calls OOS under the same cost-aware gate. ~51% accuracy (near coin-flip at 24h) and −50% to −62% OOS net of fees — both abstain. ML is a hypothesis that must earn the bar, not a black-box pass; the validated edge stays the OI-contrarian rule. Accuracy ≠ profit, confirmed with real models.", isNew: true, tone: "honest" },
  { n: 23, theme: "Method", title: "Self-learning proven on live-fetched data — and still refused the flip", body: "We fetched fresh HYPE data (a live exchange feed, unseen bars) and re-ran the gate — real new evidence, not a replay. HYPE's edge crossed: +142% OOS, walk-forward robust (fragile days earlier). The loop responded — but did NOT graduate it: a just-crossed, regime-dependent edge (HYPE's in a parabolic bull) goes on PROBATION, needing confirmation across consecutive new-data cycles before live capital (re-runs don't count — anti-overfit). Registry stays MNT-only. Promote on evidence, confirm before you trust.", isNew: true, tone: "honest" },
];
